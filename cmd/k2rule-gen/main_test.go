...
// gzips, and verifies the output is valid K2RULEV3.
...
`
	...
	if string(data[:8]) != "K2RULEV3" {
		t.Errorf("Expected magic K2RULEV3, got %q", string(data[:8]))
	}
	...
	if string(decompressed[:8]) != "K2RULEV3" {
		t.Errorf("Expected magic K2RULEV3 after decompression, got %q", string(decompressed[:8]))
	}
}

// TestGeneratePornDomains tests the generate-porn logic with a mock blocklist.
// It builds a K2RULEV3 with target=Reject and verifies the output.
func TestGeneratePornDomains(t *testing.T) {
	...
	// Build K2RULEV3 with target=Reject (2)
	...
	// Verify decompressed data has magic "K2RULEV3"
	if string(decompressed[:8]) != "K2RULEV3" {
...
// TestGzipOutputDecompressable writes a .k2r.gz file and verifies it decompresses to valid K2RULEV3.
...
// TestSliceWriterEmptyBuild verifies the SliceWriter builds a valid empty K2RULEV3.
func TestSliceWriterEmptyBuild(t *testing.T) {
	...
	if string(data[:8]) != "K2RULEV3" {
		t.Errorf("Expected K2RULEV3 magic, got %q", string(data[:8]))
	}
...
`
	...
	if string(data[:8]) != "K2RULEV3" {
		t.Errorf("Expected K2RULEV3, got %q", string(data[:8]))
	}
}
