...
type sliceData struct {
	...
	target  uint8
	domains []string
	cidrV4  []slice.CidrV4Entry
	cidrV6  []slice.CidrV6Entry
	geoips  []string
}
...
func (sd *sliceData) merge(other *sliceData) {
	...
	case kindCidrV6:
		sd.cidrV6 = append(sd.cidrV6, other.cidrV6...)
	case kindGeoIP:
	...
	for _, ruleStr := range config.Rules {
		...
		case "IP-CIDR":
			...
			slices = appendOrMerge(slices, &sliceData{
				kind:   kindCidrV4,
				target: target,
				cidrV4: []slice.CidrV4Entry{{Network: network, PrefixLen: prefixLen}},
			})
		...
		case "IP-CIDR6":
			...
			slices = appendOrMerge(slices, &sliceData{
				kind:   kindCidrV6,
				target: target,
				cidrV6: []slice.CidrV6Entry{{Network: network, PrefixLen: prefixLen}},
			})
...
func (c *SliceConverter) addProviderSlices(slices *[]*sliceData, behavior string, rules []string, target uint8) {
	...
	case "ipcidr":
		var v4 []slice.CidrV4Entry
		var v6 []slice.CidrV6Entry
		for _, rule := range rules {
			rule = strings.TrimSpace(rule)
			if rule == "" || strings.HasPrefix(rule, "#") {
				continue
			...
			if network, pfl, ok := parseCIDRv4(rule); ok {
				v4 = append(v4, slice.CidrV4Entry{Network: network, PrefixLen: pfl})
			} else if network6, pfl6, ok6 := parseCIDRv6(rule); ok6 {
				v6 = append(v6, slice.CidrV6Entry{Network: network6, PrefixLen: pfl6})
			}
		}
		if len(v4) > 0 {
			*slices = appendOrMerge(*slices, &sliceData{
	...
	case "IP-CIDR":
		if network, pfl, ok := parseCIDRv4(value); ok {
			*slices = appendOrMerge(*slices, &sliceData{
				kind:   kindCidrV4,
				target: target,
				cidrV4: []slice.CidrV4Entry{{Network: network, PrefixLen: pfl}},
			})
		}
	case "IP-CIDR6":
		if network6, pfl6, ok := parseCIDRv6(value); ok {
			*slices = appendOrMerge(*slices, &sliceData{
				kind:   kindCidrV6,
				target: target,
				cidrV6: []slice.CidrV6Entry{{Network: network6, PrefixLen: pfl6}},
			})
		}
	case "GEOIP":
...
func writeSliceData(w *slice.SliceWriter, sd *sliceData) error {
	...
	case kindCidrV6:
		return w.AddCidrV6Slice(sd.cidrV6, sd.target)
	case kindGeoIP:
